# FootballPredictions · 2足球框架 V17.4.38

体彩/竞彩足球预测框架 + 可分发 Agent Skill。  
**当前执行版 = V17.4.38**（`SKILL.md` / `output-template.md` / `references/03-复盘模板.md`）。

主菜：伤停/战意/形态/剧本定方向。佐料：盘口/SP/Edge 只做出票闸。  
方向必须给（锁\* / 主不败 / 客不败）；禁胶着；`01` 场场 `排除=`｜`剩余=`｜`二次=`。  
**V17.4.38**：【让球稳健推荐】——让球盘稳则可荐（单场或让球 B 串）；非「只能单买禁串关」。  
**V17.4.37**：场场【冷门预防】触发=是|否统一亮牌；专闸已亮禁写否；不改专闸动作。  
